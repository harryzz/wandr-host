...
impl<V: VideoFrame> VaapiDecodedHandle<V> {
    ...
    // ── wandr patch (2/2) ────────────────────────────────────────────────
    // Upstream: `pub(crate)`. Widened to `pub` so a caller can read the pixels
    // off a decoded handle's VA surface. This is the ONLY route to decoded
    // output for a custom VA-allocated `VideoFrame`: the alternative public
    // path, `VideoFrame::map()`, cannot work for us because `to_native_handle`
    // hands the `Surface` to the backend by value and `Surface` is not
    // cloneable, so the frame no longer has it to map. See
    // crates/wandr-video/src/backends/vaapi.rs and WANDR-PATCHES.md.
